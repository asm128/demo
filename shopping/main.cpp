...
::gpk::error_t										gpk_cgi_generate_output			(::gpk::SCGIRuntimeValues & runtimeValues, ::gpk::array_pod<char_t> & output)	{
	...
	if(lang == ::gpk::view_const_string{"es"})
		title = (::gpk::view_const_string{ "Comercios en Obelisco"});
	else
		title = (::gpk::view_const_string{ "Shopping in Obelisco"});

	::ntl::pageCatalog("ads.json", programState.Path.Style, ::ntl::AD_SHOP_CATEGORY_shops, title, lang, output);
